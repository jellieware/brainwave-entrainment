...
function chime_soundr() {
    ...
    play -q -n -b 24 -r 44100   -c1  synth "$duration" sine "$freq" fade 0.001 0 0.2 "$duration" lowpass 100 vol 1 remix 0 1 > /dev/null 2>&1 &
}
function chime_soundl() {
    ...
    play -q -n -b 24 -r 44100  -c1  synth "$duration" sine "$freq" fade 0.001 0 0.2 "$duration" lowpass 100 vol 1 remix 1 0 > /dev/null 2>&1 &
}
...
play -q  -n synth brownnoise gain -20 vol 0.4 reverb 50 50 100 100 20 lowpass 200 highpass 600 &
...
DURATION=$( echo "scale=2; 0.2 + 0.3 * $RANDOM / 32767" | bc )

RANDOM_NUMBER=$((RANDOM % RANGE + MIN))
play -q -n -b 24 -r 44100   synth $DURATION \
  ...
  sleep $( echo "scale=2; 0.5 + 1 * $RANDOM / 32767" | bc )
