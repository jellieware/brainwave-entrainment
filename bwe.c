...
void 
init_organic_blur_static_safe (void)
{
  if (blur_initialized)
    return;
...
void
blur_bubbles_engine (int16_t *buffer, int frames)
{
  if (!blur_initialized)
    {
...
float minnaert_freq(float r) {
    if (r <= 0.00001f) return 20000.0f;
    return (1.0f / (2.0f * (float)M_PI * r)) * sqrtf((3.0f * 1.4f * 101325.0f) / 1000.0f);
}
...
void trigger_crystal(float energy, float pan) {
    for (int i = 0; i < MAX_BUBBLES; i++) {
        if (!pool[i].active) {
            ...
            // Map the size parameters dynamically between the defined min and max variables
...
void render_samples(float *l, float *r) {
    *l = 0.0f; *r = 0.0f;
    ...
    for (int i = 0; i < MAX_BUBBLES; i++) {
        if (pool[i].active) {
            float current_radius = pool[i].base_radius * (1.0f - 0.12f * logf(1.0f + (pool[i].age * FLUID_SPEED) * 40.0f));
...
static void* run_waveguide_mesh_engine(void* arg) {
    ...
    snd_pcm_hw_params_set_format(handle, params, SND_PCM_FORMAT_S16_LE); // Strict native 16-Bit S16_LE 
    ...
    const float blur_alpha = 0.05f; // Lower alpha creates an increasingly blurred, liquid tone

    while (ctrl->keep_running) {
        for (int i = 0; i < STREAM_BUFFER_SIZE; i++) {
            
            // 1. Stochastic Energy Injection: Push random friction impulses into the mesh core
            // This constantly triggers the physical waveguide junctions
            mesh_pos[MESH_ROWS / 2][MESH_COLS / 2] += fast_white_noise() * 0.12f;

            // 2. Compute 2D Digital Scattering Wave Equations across neighbors
            // Wave propagation speed constant (Courant stability criterion: c <= 0.5 for 2D grids)
            ...
            for (int r = 1; r < MESH_ROWS - 1; r++) {
                for (int c = 1; c < MESH_COLS - 1; c++) {
                    ...
                    float spatial_acceleration = (mesh_pos[r+1][c] + mesh_pos[r-1][c] + 
                                                  mesh_pos[r][c+1] + mesh_pos[r][c-1] - 
                                                  4.0f * mesh_pos[r][c]);
            ...
            // 4. Acoustic Spatial Selection: Sample multiple distinct areas of the mesh grid
            // This replicates an acoustic microphone tracking a large, moving physical space
            ...
            // 5. Apply Exponential Moving Average (EMA) to blur the grid signals into fluid motion
            low_pass_blur = (blur_alpha * mesh_output_mix) + ((1.0f - blur_alpha) * low_pass_blur);

            // Safe normalize transformation
            float final_signal = low_pass_blur * 0.15f * VOLUME_SCALE;
            if (final_signal > 1.0f)  final_signal = 1.0f;
            if (final_signal < -1.0f) final_signal = -1.0f;
...
float
sph_cubic_spline_kernel (float distance, float h)
{
...
void
smooth_bubbles_sph (int16_t *buffer, int frames, float smoothing_radius)
{
      ...
      for (int f = 0; f < frames; f++)
	{
	  ...
	  for (int neighbor = -SPH_WINDOW_SIZE; neighbor <= SPH_WINDOW_SIZE;
	      ...
	      if (target_frame >= 0 && target_frame < frames)
		{
		  ...
		  float weight =
		    sph_cubic_spline_kernel (distance, smoothing_radius);
...
#define SAMPLE_RATE 44100
#define PERIOD_SIZE 512         // Increased slightly to provide structural wake-up cushion
...
void spawn_bounded_bubble(float volume, float speed, float min_radius, float max_radius) {
        ...
        //    bubble_poolyyy[i].minnaert_freq = freq + (((float)rand() / RAND_MAX) * 300.0f - 150.0f);
         ...
         float freq = ( 1.0f / ( 2.0f * M_PI * a)) * sqrtf(( 3.0f * gamma * p_A) / rho);
          ...
          //  if (bubble_poolyyy[i].minnaert_freq < 50.0f) bubble_poolyyy[i].minnaert_freq = 50.0f;
            ...
            bubble_poolyyy[i].decay_rate = (30.0f + ((float)rand() / RAND_MAX) * 60.0f) * (1.0f / (size_factor + 0.5f));
...
void* loop_one(void* arg) {
    ...
    for (int i = 0; i < MAX_ACTIVE_BUBBLES; i++) bubble_poolyyy[i].active = 0;
...
void* loop_two(void* arg) {
  ...
  while (1)
      ...
      for (int f = 0; f < BUFFER_FRAMES; f++)
	{
	  ...
	  carrier_phase_left +=
	    (2.0 * M_PI * carrier_freq_left) / SAMPLE_RATE;
	  carrier_phase_right +=
	    (2.0 * M_PI * carrier_freq_right) / SAMPLE_RATE;
	  ...
	  for (int i = 0; i < NUM_DROPLETS; i++)
	      ...
	      if (droplets[i].active)
		{
		  ...
		  mixed_left +=
		    sample_mono * droplets[i].pan_left * MASTER_VOLUME;
		  mixed_right +=
		    sample_mono * droplets[i].pan_right * MASTER_VOLUME;
		  droplets[i].age += dt;
		}
	    }
	  ...
	  float random_roll_R =
	    (float) ((noise_state >> 16) & 0xFFFF) / 65535.0f;
	  ...
	  for (int v = 0; v < MAX_POLYPHONY; v++)
	      ...
	      if (drops_L[v].active)
		{
		  ...
		  float window =
		    sinf (3.14159265f * progress) * (1.0f - progress);
		  float local_amp = drops_L[v].amp_envelope * window;
		  float dynamic_freq =
		    drops_L[v].base_freq * (1.0f +
	      ...
	      if (drops_R[v].active)
		{
		  ...
		  float dynamic_freq =
		    drops_R[v].base_freq * (1.0f +
					    (progress * progress * 1.6f));
		  scholastic_water_R +=
		    sinf (drops_R[v].age *
			  (2.0f * 3.14159265f * dynamic_freq / 44100.0f)) *
		    local_amp;
...
void trigger_minnaert_bubble(float grid_energy, int x, int y) {
    for (int i = 0; i < MAX_BUBBLES; i++) {
        if (!bubble_pool[i].active) {
            ...
            bubble_pool[i].decay = expf(-0.04f * bubble_pool[i].frequency / SAMPLE_RATE); 
    ...
    for (int i = 0; i < MAX_BUBBLES; i++) {
        if (bubble_pool[i].active) {
            ...
            *out_right += wave * bubble_pool[i].pan;
            bubble_pool[i].phase += 2.0f * (float)M_PI * bubble_pool[i].frequency / SAMPLE_RATE;
            bubble_pool[i].amplitude *= bubble_pool[i].decay;
            if (bubble_pool[i].amplitude < 0.001f) bubble_pool[i].active = 0;
        }
    }
}
