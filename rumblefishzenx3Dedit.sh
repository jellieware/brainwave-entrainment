...
function chime_sound() {
    ...
    play -q -n -c1 synth "$duration" sine "$freq" fade l 0 0.2 "$duration" lowpass 100 vol 0.5 > /dev/null 2>&1 &
}
...
play -q -n synth \
     ...
     reverb 40 50 100 pitch -31.76665 vol 0.02 > /dev/null 2>&1 & 
   play -q -n -c2 synth pinknoise mix band -n 9000 1000 tremolo 2000 1 lowpass 1000 reverb 40 50 100 pitch -31.76665 vol 0.5 > /dev/null 2>&1 &
...
if [[ "$input_char" == "q" ]]; then
    ...
    sleep 2
    pkill play
    pkill -f "rumblefishzenx3Dedit.sh"
    sleep 2
    xkill
    sleep 2
