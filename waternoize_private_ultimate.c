  ...
  for (int i = 0; i < NUM_DROPLETS; i++) {
    if (!droplets[i].active) {
      ...
      droplets[i].sweep_start =
          rand_double(20, 100.0) * size_factor + micro_drift;
...
void update_micro_splash_grid(float volume, float speed, float min_r, float max_r) {
    ...
    if ((rand() % 100) < 8) { // 8% chance per frame keeps it clean
        int drop_x = 1 + rand() % (GRID_SIZEZ - 2);
        int drop_y = 1 + rand() % (GRID_SIZEZ - 2);
        current_gridz[drop_x][drop_y] = 2.0f;
    }

    // 3. Scan for crest triggers with active cooldown lockouts
    for (int x = 1; x < GRID_SIZEZ - 1; x++) {
        for (int y = 1; y < GRID_SIZEZ - 1; y++) {
            // Cool down tracking
            if (grid_cooldown[x][y] > 0) {
                grid_cooldown[x][y]--;
            }

            if (current_gridz[x][y] > 1.4f && grid_cooldown[x][y] == 0) {
                // Micro-splashes must scale smaller (higher frequencies, shorter lifespans)
                float micro_min_r = min_r * 0.4f; 
                float micro_max_r = max_r * 0.5f;

                spawn_bounded_bubblex(volume, speed, micro_min_r, micro_max_r);
                
                // Block this cell from firing another sound for 15 audio frames
                grid_cooldown[x][y] = 15; 
                
                // Disperse kinetic energy locally so it doesn't loop
                current_gridz[x][y] *= 0.2f; 
            }
        }
    }
}
...
void *loop_three(void *arg) {
 ...
 float min_splash_radius = 0.0019f; // Increased from 0.0019f to eliminate sharp clicks
float max_splash_radius = 0.090f; // Increased from 0.090f for deeper resonant bodies
