...
for (int x = 1; x < GRID_SIZEZ - 1; x++) {
    for (int y = 1; y < GRID_SIZEZ - 1; y++) {
        ...
        if (current_gridz[x][y] > 3.0f) current_gridz[x][y] = 0.0f;
        else if (current_gridz[x][y] < -3.0f) current_gridz[x][y] = -0.0f;
    }
}
...
double DROPLET_MIN_RADIUS = 0.0004; // Slightly larger minimum eliminates "sand/wall" texture
double DROPLET_MAX_RADIUS = 0.050; //
typedef struct {
    int active;
    double amplitude;     
    double base_freq;     // Target Minnaert frequency
    double time;          // Keeps track of time for smooth continuous pitch glissando
    double decay;         
    double current_amp;   
    double pan_left;      
    double pan_right;
    double phase;         // Replaces static resonators to allow smooth, sweeping pitch
} Bubblevvv;
...
void trigger_fluid_bubble() {
    for (int i = 0; i < MAX_BUBBLES; i++) {
        if (!bubble_poolvvv[i].active) {
            ...
            // Biased distribution so we get a good mix of deep background chugs and clear surface bloops
            ...
            if (bubble_poolvvv[i].base_freq > 14000.0) bubble_poolvvv[i].base_freq = 14000.0;
            if (bubble_poolvvv[i].base_freq < 60.0) bubble_poolvvv[i].base_freq = 60.0;

            // Fluid Damping: Higher frequencies fade much faster. Deep thuds ring longer.
            double damping_rate = 0.12 * bubble_poolvvv[i].base_freq + 25.0;
            bubble_poolvvv[i].decay = exp(-damping_rate / SAMPLE_RATE);
            
            // Scaled down volume to allow huge overlapping liquid walls
...
void *loop_four(void *arg) {
  snd_pcm_t *handle;
    ...
    if ((err = snd_pcm_set_params(handle, SND_PCM_FORMAT_S16_LE, SND_PCM_ACCESS_RW_INTERLEAVED, CHANNELS, SAMPLE_RATE, 1, 40000)) < 0) {
        ...
        for (int i = 0; i < buffer_size; i++) {
            ...
            for (int b = 0; b < MAX_BUBBLES; b++) {
                if (bubble_poolvvv[b].active) {
                    ...
                    // WATER PHYSICS: Upward pitch glissando sweep.
                    // The bubble frequency shoots up exponentially by 40% over the first few milliseconds.
                    // This mimics the shrinking/stabilizing air cavity under water and creates the distinct "bloop".
                    double pitch_sweep = 1.0 + 0.40 * exp(-140.0 * t);
                    double dynamic_freq = bubble_poolvvv[b].base_freq * pitch_sweep;

                    // Accumulate phase smoothly across the changing pitch sweep
                    bubble_poolvvv[b].phase += (2.0 * PI * dynamic_freq) / SAMPLE_RATE;
                    
                    // Generate wave with a slight non-linear distortion characteristic of fluid boundary walls
                    double wave = sin(bubble_poolvvv[b].phase + 0.05 * sin(bubble_poolvvv[b].phase));
                    
                    // Track structural volume decay
            ...
            // High-density liquid audio soft limiter
            ...
            buffer[i * CHANNELS + 1] = (short)(right_sample * MASTER_VOLUMEV* 32767.0);
        }
