...
void trigger_droplet() {
    for (int i = 0; i < NUM_DROPLETS; i++) {
        if (!droplets[i].active) {
            ...
            droplets[i].duration = rand_double(0.02, 0.18);
            ...
            droplets[i].sweep_start = rand_double(50.0, 150.0);
            
            // Sweep target climbs swiftly away from bass rumble to create clean fluid definition
            droplets[i].sweep_end = droplets[i].sweep_start + rand_double(1200.0, 1650.0);
...
int main() {
    ...
    printf("Now Streaming: Synthetic Rain...\n");
    ...
    while (1) {
            ...
            for (int i = 0; i < NUM_DROPLETS; i++) {
                if (droplets[i].active) {
                    ...
                    if (droplets[i].current_phase > 2.0 * PI) {
                        droplets[i].current_phase = fmod(droplets[i].current_phase, 2.0 * PI);
                    }
        ...
        snd_pcm_sframes_t written = snd_pcm_writei(pcm_handle, buffer, BUFFER_FRAMES);
        if (written < 0) {
