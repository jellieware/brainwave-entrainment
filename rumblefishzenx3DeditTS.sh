...
function chime_sound() {
    ...
    play -q -n -c1 synth "$duration" sine "$freq" fade 0.001 0 0.2 "$duration" lowpass 100 vol 0.5 > /dev/null 2>&1 &
}
...
play -q -n synth \
     ...
     reverb 40 50 100 vol 0.02 > /dev/null 2>&1 & 
...
play -q -n synth whitenoise bandpass $CENTER_FREQ $BANDWIDTH lowpass 50 vol 0.5 &
